...
mod tests {
  ...
  fn test_save_and_load_poll() {
    ...
    // Load poll
    let loaded_poll = store.load_poll(&poll_id).unwrap().unwrap();
  ...
  fn test_load_nonexistent_poll() {
    ...
    let result = store.load_poll(&nonexistent_id).unwrap();
  ...
  fn test_remove_poll() {
    ...
    // Verify it exists
    assert!(store.load_poll(&poll_id).unwrap().is_some());

    // Remove poll
    store.remove_poll(&poll_id).unwrap();

    // Verify it's gone
    assert!(store.load_poll(&poll_id).unwrap().is_none());
  }
  ...
  fn test_remove_checkin_config() {
    ...
    store.remove_checkin_config(guild_id).unwrap();
  ...
  fn test_multiple_operations() {
    ...
    // Load both
    let loaded_poll = store.load_poll(&poll_state.id).unwrap().unwrap();
    ...
    assert!(store.load_poll(&poll_state.id).unwrap().is_none());
    assert!(store
      .load_checkin_config(checkin_ctx.guild_id)
      .unwrap()
      .is_none());
